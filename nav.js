...
.ynb-drawer-sub-link--nested { padding-left: 1.4rem !important; }
...
.ynb-lang-btn:hover { border-color: #8B4513; color: #8B4513; background: rgba(139,69,19,0.05); }
.ynb-lang-btn svg { width: 14px; height: 14px; flex-shrink: 0; opacity: 0.7; }
.ynb-lang-btn .ynb-lang-chevron { width: 8px; height: 8px; opacity: 0.5; transition: transform 0.2s ease; }
...
.ynb-lang-option {
  ...
  font-family: 'Lato', sans-serif;
  font-size: 0.78rem; color: #38322a;
...
.ynb-lang-option.ynb-lang-active { color: #8B4513; font-weight: 700; }
...
.ynb-drawer-lang-label {
  ...
  font-size: 0.58rem; font-weight: 600;
  letter-spacing: 0.16em; text-transform: uppercase;
  color: #8B4513; margin-bottom: 0.6rem; display: block;
}
.ynb-drawer-lang-grid { display: flex; flex-wrap: wrap; gap: 0.4rem; }
.ynb-drawer-lang-opt {
  padding: 0.35rem 0.75rem;
  border: 1px solid #c8bfb0; border-radius: 4px;
  font-family: 'Lato', sans-serif;
  font-size: 0.72rem; color: #38322a;
...
`;
  ...
  function chevronSVG(cls) {
    ...
    svg.setAttribute('viewBox','0 0 10 6'); svg.setAttribute('fill','none');
    svg.setAttribute('aria-hidden','true'); svg.setAttribute('class',cls);
    const p = document.createElementNS('http://www.w3.org/2000/svg','path');
    p.setAttribute('d','M1 1l4 4 4-4'); p.setAttribute('stroke','currentColor');
  ...
  function globeSVG() {
    ...
    svg.innerHTML = '<circle cx="12" cy="12" r="10"/><path d="M2 12h20M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/>';
    return svg;
  }

  /* ─────────────────────────────────────────────
     3. SITEMAP — fetch with localStorage cache
  ───────────────────────────────────────────── */
  function getCached() {
    try {
  ...
  async function fetchSitemap() {
    ...
    if (cached) {
      // Serve instantly from cache, refresh silently in background
      fetch(SITEMAP).then(r => r.ok && r.text()).then(xml => xml && setCache(xml)).catch(()=>{});
      return cached;
    }
    // No cache — fetch now, try same-origin first then proxies
    try {
      const r = await fetch(SITEMAP);
      if (r.ok) { const xml = await r.text(); setCache(xml); return xml; }
    } catch (_) {}
    try {
      const r = await fetch('https://api.allorigins.win/get?url=' + encodeURIComponent(SITEMAP));
      if (r.ok) { const j = await r.json(); if (j?.contents) { setCache(j.contents); return j.contents; } }
    } catch (_) {}
    try {
      const r = await fetch('https://corsproxy.io/?' + encodeURIComponent(SITEMAP));
      if (r.ok) { const xml = await r.text(); setCache(xml); return xml; }
    } catch (_) {}
    return null;
  }
  ...
  function parseSitemap(xml) {
    ...
    while ((m = re.exec(xml)) !== null) {
      ...
      if (parts.length === 1) {
        ...
        cats.get(cat).push({ name: toTitleCase(parts[parts.length-1]), url, group: toTitleCase(parts[1]) });
      }
    }
    return cats;
  }
  ...
  function nukeGTCookies() {
    ...
    ['', hostname, '.'+hostname, bare, '.'+bare].forEach(d => {
      const dc = d ? 'domain='+d+'; ' : '';
      ['', '; SameSite=Lax', '; SameSite=None; Secure'].forEach(ss => {
        document.cookie = 'googtrans=; path=/; ' + dc + exp + ss;
      });
    });
  }
  ...
  function loadGoogleTranslate() {
    ...
    window.googleTranslateElementInit = function () {
      new google.translate.TranslateElement({ pageLanguage:'en', autoDisplay:false, gaTrack:false }, 'google_translate_element');
    };
  ...
  function buildLangMenu() {
    ...
    lbl.setAttribute('translate','no'); lbl.textContent = currentLang.label;
    btn.appendChild(lbl);
    ...
    LANGUAGES.forEach(lang => {
      ...
      opt.setAttribute('role','menuitem'); opt.setAttribute('translate','no');
    ...
    btn.addEventListener('click', e => {
      ...
      if (!isOpen) { wrap.classList.add('ynb-lang-open'); btn.setAttribute('aria-expanded','true'); }
    });
    document.addEventListener('click', () => { wrap.classList.remove('ynb-lang-open'); btn.setAttribute('aria-expanded','false'); });
    document.addEventListener('keydown', e => { if (e.key==='Escape') { wrap.classList.remove('ynb-lang-open'); btn.setAttribute('aria-expanded','false'); } });
    wrap.appendChild(btn); wrap.appendChild(panel);
    return wrap;
  }
  function buildDrawerLangSection(inner) {
    const current = getCurrentLang();
    ...
    LANGUAGES.forEach(lang => {
      ...
      btn.setAttribute('translate','no'); btn.textContent = lang.label;
  ...
  function buildDesktopItems(list, cats) {
    ...
    function closeAll() {
      ...
      if (openLi)   { openLi.classList.remove('ynb-open'); openLi.querySelector('.ynb-trigger')?.setAttribute('aria-expanded','false'); openLi = null; }
    }
    cats.forEach((subs, cat) => {
      ...
      if (subs.length === 0) {
        ...
        subs.forEach(p => { const g=p.group||''; if (!groupMap.has(g)) groupMap.set(g,[]); groupMap.get(g).push(p); });
        ...
        groups.forEach(([group, pages]) => {
          ...
          pages.forEach(p => linksDiv.appendChild(el('a', { className:'ynb-link', href:p.url, textContent:p.name })));
          colsDiv.appendChild(el('div', { className:'ynb-col' }, [
            el('span', { className:'ynb-cat-label', textContent: group || label }),
            linksDiv,
          ]));
        });
        ...
        btn.addEventListener('click', e => {
          ...
          if (!isOpen) {
            ...
            drop.style.top = NAV_H+'px'; drop.style.left = left+'px'; drop.style.width = dropW+'px';
            drop.classList.add('ynb-visible'); li.classList.add('ynb-open');
            btn.setAttribute('aria-expanded','true'); openDrop=drop; openLi=li;
          }
        });
      }
      list.appendChild(li);
    ...
    cats.forEach((subs, cat) => {
      ...
      if (subs.length === 0) {
        ...
        groupMap.forEach((pages, group) => {
          if (group) sub.appendChild(el('div', { className:'ynb-drawer-subfolder', textContent:group }));
          pages.forEach(p => sub.appendChild(el('a', {
            className: 'ynb-drawer-sub-link' + (group ? ' ynb-drawer-sub-link--nested' : ''),
            href: p.url, textContent: p.name,
          })));
        });
        ...
        trig.addEventListener('click', () => {
          ...
          inner.querySelectorAll('.ynb-drawer-item.ynb-drawer-open').forEach(i => {
            i.classList.remove('ynb-drawer-open'); i.querySelector('button')?.setAttribute('aria-expanded','false');
          });
          if (!isOpen) { item.classList.add('ynb-drawer-open'); trig.setAttribute('aria-expanded','true'); }
        });
        item.appendChild(trig); item.appendChild(sub);
      }
      inner.appendChild(item);
    });
    buildDrawerLangSection(inner);
    drawer.innerHTML = ''; drawer.appendChild(inner);
  }
  ...
  function injectNav() {
    ...
    nav.setAttribute('role','navigation'); nav.setAttribute('aria-label','Site navigation');
    ...
    [72,60,88,56].forEach(w => { const pill=el('div',{className:'ynb-skel-pill'}); pill.style.width=w+'px'; skel.appendChild(pill); });
    list.appendChild(skel);
    nav.appendChild(list);
    nav.appendChild(buildLangMenu());
    const ham = el('button', { className:'ynb-hamburger', 'aria-label':'Open navigation menu', 'aria-expanded':'false' });
    ...
    document.body.style.paddingTop = (parseInt(document.body.style.paddingTop,10)||0) + NAV_H + 'px';
    function openDrawer()  { drawer.classList.add('ynb-drawer-open'); scrim.classList.add('ynb-scrim-visible'); ham.classList.add('ynb-active'); ham.setAttribute('aria-expanded','true'); document.body.style.overflow='hidden'; }
    function closeDrawer() { drawer.classList.remove('ynb-drawer-open'); scrim.classList.remove('ynb-scrim-visible'); ham.classList.remove('ynb-active'); ham.setAttribute('aria-expanded','false'); document.body.style.overflow=''; }
    ham.addEventListener('click', e => { e.stopPropagation(); drawer.classList.contains('ynb-drawer-open') ? closeDrawer() : openDrawer(); });
